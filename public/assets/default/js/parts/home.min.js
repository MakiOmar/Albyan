(function ($) {
  ...
  document.querySelectorAll('.category-courses-swiper').forEach(function (container) {
    ...
    var paginationEl = section ? section.querySelector('.category-courses-swiper-pagination') : null;
    if (!paginationEl) return;
    var swip = new Swiper(container, {
      slidesPerView: 1,
      spaceBetween: 16,
      loop: false,
      autoplay: {
  ...
  $(document).ready(() => {
    Array.from({ length: 6 }, (_, i) => {
      const element = document.getElementById(`parallax${i + 1}`);
      if (element) {
        new Parallax(element, { relativeInput: true });
      }
    });
  });
})(jQuery);
