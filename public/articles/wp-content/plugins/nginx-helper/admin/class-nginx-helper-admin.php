...
class Nginx_Helper_Admin {
	...
	/**
	 * Key used to persist purge debug messages between redirect requests.
	 */
	const DEBUG_NOTICE_TRANSIENT_KEY = 'rt_wp_nginx_helper_debug_notices';
	...
	public function purge_all() {
		...
		$nginx_purger->log( 'Purge request check started on admin_bar_init hook.' );
		...
		if ( empty( $action ) ) {
			$nginx_purger->log( 'Purge request ignored because nginx_helper_action is empty.' );
			return;
		}
		
		$nginx_purger->log( 'Purge request detected. Action: ' . $action . ', method: ' . $method );
		
		if ( ! current_user_can( 'Nginx Helper | Purge cache' ) ) {
			$nginx_purger->log( 'Purge blocked: current user does not have "Nginx Helper | Purge cache" capability.', 'ERROR' );
		...
		$nginx_purger->log( 'Purge request nonce validated. Current URL: ' . $current_url );
		...
		switch ( $action ) {
			case 'purge':
				$nginx_purger->log( 'Executing full purge operation.' );
				$nginx_purger->purge_all();
				break;
			case 'purge_current_page':
				$nginx_purger->log( 'Executing current page purge operation for URL: ' . $current_url );
		...
		if ( 'purge' === $action ) {
			...
			$nginx_purger->log( 'Full purge operation finished and rt_nginx_helper_after_purge_all fired.' );
			
		}
		
		if ( is_object( $nginx_purger ) && method_exists( $nginx_purger, 'get_debug_messages' ) ) {
			$this->store_purge_debug_notices( $nginx_purger->get_debug_messages() );
		}
		
		$nginx_purger->log( 'Purge flow complete. Redirecting to: ' . $redirect_url );
	...
	public function display_notices() {
		$debug_messages = $this->get_purge_debug_notices();
		
		echo '<div class="updated"><p>' . esc_html__( 'Purge initiated', 'nginx-helper' ) . '</p>';
		
		if ( ! empty( $debug_messages ) && is_array( $debug_messages ) ) {
			echo '<p><strong>' . esc_html__( 'Purge debug log:', 'nginx-helper' ) . '</strong></p>';
			echo '<div style="max-height:240px;overflow:auto;background:#fff;border:1px solid #ccd0d4;padding:8px 10px;">';
			echo '<ul style="margin:0;list-style:disc;padding-left:18px;">';
			
			foreach ( $debug_messages as $debug_message ) {
				echo '<li><code>' . esc_html( $debug_message ) . '</code></li>';
			}
	...
	/**
	 * Store purge debug messages for next request after redirect.
	 *
	 * @param array $messages Debug messages.
	 * @return void
	 */
	private function store_purge_debug_notices( $messages ) {
		if ( ! is_array( $messages ) || empty( $messages ) ) {
			return;
		}
		
		set_site_transient( self::DEBUG_NOTICE_TRANSIENT_KEY, array_values( $messages ), 5 * MINUTE_IN_SECONDS );
	}
	
	/**
	 * Retrieve and clear purge debug messages.
	 *
	 * @return array
	 */
	private function get_purge_debug_notices() {
		$messages = get_site_transient( self::DEBUG_NOTICE_TRANSIENT_KEY );
		delete_site_transient( self::DEBUG_NOTICE_TRANSIENT_KEY );
