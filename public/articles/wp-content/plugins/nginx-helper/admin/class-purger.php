...
abstract class Purger {
	...
	/**
	 * In-memory debug messages for current request.
	 *
	 * @var array
	 */
	...
	protected function delete_cache_file_for( $url ) {
		...
		// Build hash candidates for common FastCGI cache key variants.
		$url_path       = isset( $url_data['path'] ) ? $url_data['path'] : '';
		$url_query      = isset( $url_data['query'] ) && '' !== $url_data['query'] ? '?' . $url_data['query'] : '';
		$request_uri    = $url_path . $url_query;
		$hash_candidates = array(
			// Backward-compatible with previous plugin behavior.
			md5( $url_data['scheme'] . 'GET' . $url_data['host'] . $url_path ),
			// Common nginx fastcgi_cache_key: "$scheme$request_method$host$request_uri".
			md5( $url_data['scheme'] . 'GET' . $url_data['host'] . $request_uri ),
		);
		$hash_candidates = array_unique( $hash_candidates );
		...
		if ( ! is_dir( $cache_path ) ) {
			$this->log( 'Cache path does not exist or is not a directory: ' . $cache_path, 'ERROR' );
			return false;
		}

		$checked_files = array();

		foreach ( $hash_candidates as $hash ) {
			...
			if ( unlink( $cached_file ) ) {
				$this->log( '- - ' . $url . ' *** PURGED *** ( matched cache file: ' . $cached_file . ' )' );
			...
			$this->log( '- - An error occurred deleting the cache file. Check filesystem permissions. File: ' . $cached_file, 'ERROR' );
			return false;
		}
		
		$this->log( '- - ' . $url . ' is currently not cached ( checked files: ' . implode( ', ', $checked_files ) . ' )' );
		return false;
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( $response['response']['code'] ) {
				...
				switch ( $response['response']['code'] ) {
					...
					case 404:
						...
						break;
					default:
						$status_message = isset( $response['response']['message'] ) ? $response['response']['message'] : 'No response message';
						$this->log( '- - ' . $url . ' purge response was unexpected ( ' . $response['response']['code'] . ' - ' . $status_message . ' )', 'WARNING' );
						$body_excerpt = wp_remote_retrieve_body( $response );
						if ( ! empty( $body_excerpt ) ) {
							$body_excerpt = substr( wp_strip_all_tags( $body_excerpt ), 0, 200 );
							$this->log( '- - Remote purge body excerpt: ' . $body_excerpt, 'WARNING' );
						}
			...
			if ( isset( $response['headers'] ) && is_object( $response['headers'] ) && method_exists( $response['headers'], 'getAll' ) ) {
				...
				if ( isset( $headers['x-cache-status'] ) ) {
					$cache_status_header = is_array( $headers['x-cache-status'] ) ? implode( ', ', $headers['x-cache-status'] ) : $headers['x-cache-status'];
				} elseif ( isset( $headers['x-fastcgi-cache'] ) ) {
					$cache_status_header = is_array( $headers['x-fastcgi-cache'] ) ? implode( ', ', $headers['x-fastcgi-cache'] ) : $headers['x-fastcgi-cache'];
				}
			}
			
			if ( ! empty( $cache_status_header ) ) {
				$this->log( '- - Upstream cache status header: ' . $cache_status_header );
			}
	...
	public function log( $msg, $level = 'INFO' ) {
		...
		if ( $log_levels[ $level ] >= $log_levels[ $nginx_helper_admin->options['log_level'] ] ) {
			$this->debug_messages[] = gmdate( 'Y-m-d H:i:s ' ) . ' | ' . $level . ' | ' . $msg;

			$log_dir  = $nginx_helper_admin->functional_asset_path();
			...
			if ( $fp ) {
				...
				error_log( 'Nginx Helper log write failed for file: ' . $log_file . ' | ' . $level . ' | ' . $msg ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				return false;
	...
	/**
	 * Retrieve debug messages collected during current request.
	 *
	 * @return array
	 */
	...
	public function unlink_recursive( $dir, $delete_root_too ) {
		...
		if ( ! is_dir( $dir ) ) {
			$this->log( 'unlink_recursive skipped because directory does not exist: ' . $dir, 'WARNING' );
			return;
		}
		...
		while ( false !== ( $obj = readdir( $dh ) ) ) {
			...
			if ( ! @unlink( $target ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
				$this->unlink_recursive( $target, false );
			}
		}
