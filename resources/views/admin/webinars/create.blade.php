...
@push('scripts_bottom')
    <script>
        $(document).ready(function () {
            // Convert minutes to hours on document ready
            const durationInput = $("input[name='duration']");
            const durationHoursInput = $("input[name='duration-hours']");

            const duration = parseInt(durationInput.val(), 10);
            if (!isNaN(duration)) {
                durationHoursInput.val(duration / 60); // Set hours equivalent
            }

            // Update duration field when duration-hours changes
            durationHoursInput.on("input", function () {
                const hours = parseFloat($(this).val());
                if (!isNaN(hours)) {
                    durationInput.val(Math.round(hours * 60)); // Set minutes equivalent
                }
            });
        });
